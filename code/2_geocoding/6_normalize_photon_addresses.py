...
# Import data
addresses_data_frame = pd.read_csv("YOUR_DIR/geocoded_addresses_photon.csv", usecols = ['addr_id', 'addr_string.photon'])

# Process and store in a list
addresses_photon_processed = addresses_data_frame.progress_apply(
    process_address_row, 
    axis=1,
    id_field="addr_id",
    address_field="addr_string.photon",
    pars_arg=pars
)

# To a DataFrame
addresses_photon_processed = pd.DataFrame(addresses_photon_processed.explode().tolist())

# Reset index
addresses_photon_processed.reset_index(drop=True, inplace=True)

# Save
addresses_photon_processed.to_csv("YOUR_DIR/pullenti_addr_photon.csv")
