...
- About 380,000 new statements for 2011–2023 have also been included:
    - Approximately 12,000 statements were either added to GIR BO retrospectively or lost in version 1.0.1 due to the absence of the corresponding organizations in our previous EGRUL panel (which has now been updated).
    - Around 68,000 statements from 2023 were reconstructed from the 2024 data.
    - The remaining 300,000+ statements are those that we had reconstructed before but erroneously dropped due to a bug in previous versions (see Fixed).
...
- Enhanced geocoding quality: an improved data cleaning procedure has enabled us to refine geocoding from the city level to the street level for firms that generated approximately 8% of total revenue from 2022 onwards, coinciding with EGRUL's transition from KLADR to FIAS. See below:
